if (typeof exports !== "undefined") {
  ...
  globalThis["serverWs"] = bar
}

function bar(x,n) {
	require("dotenv").config()
	global["RELOAD_TIME"] = 65
	function makeid(x) {
		let text = ""
		let possible = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
		for ( let i=0;i<x;i++ ) text += possible.charAt( Math.floor( Math.random() * (possible.length + 1) ) )
		return text
	}
	...
	app.get("/",(req, res) => {
		...
		socket.emit("browser_name", fnBrowserDetect() )
